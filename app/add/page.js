...
function OnBoardingPage() {
  ...
  const checkDomainAddedBefore = async () => {
    ...
    if (
      fetchedWebites.filter((item) => item.website_name == website).length > 0
    ) {
  ...
  useEffect(() => {
    if (
      ...
      setError("Enter the domain onyl. ie:(google.com)");
    } else {
      setError("");
    }
  }, [website]);
          ...
          <div className="w-full items-center justify-center flex flex-col space-y-10">
            <span className="w-full lg:w-[50%]">
              <textarea
                ...
                src="https://analytics3.vercel.app/tracking-script.js"></script>`}
              />
              <p className="text-xs text-white/20 pt-2 font-light">
